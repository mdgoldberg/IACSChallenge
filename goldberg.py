...
Xdf = comp_df.ix[:, ['Xcoord', 'Ycoord', 'hour', 'neighbor_avg', 'isRestroom', 'isStaircase'] +
                 ...
                 # or c in sens_dummies.columns
                 ]
Y = comp_df.value.copy()

# model = ensemble.RandomForestRegressor(n_jobs=-1)
# grid = {'n_estimators': [100, 150, 200], 'max_features': ['auto', 'sqrt', 'log2', 0.4, 0.5, 0.6]}
model = ensemble.GradientBoostingRegressor(loss='huber', n_estimators=150, max_depth=5)
grid = {'learning_rate': [0.05, 0.1, 0.15, 0.2], 'alpha': [0.8, 0.85, 0.9, 0.95, 0.99]}
